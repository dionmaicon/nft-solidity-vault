...
const getMintNFTFixture = async () => {
  ...
  nfts.push(
    await mintNFT(
      ACCOUNTS_INDEX.DEPLOYER,
      ACCOUNTS_INDEX.FIRST_ACCOUNT,
      "https://gateway.pinata.cloud/ipfs/QmZAh_FIRST_ACCOUNT",
      gameItem
    )
  );
  nfts.push(
  ...
  nfts.push(
    await mintNFT(
      ACCOUNTS_INDEX.DEPLOYER,
      ACCOUNTS_INDEX.SECOND_ACCOUNT,
      "https://gateway.pinata.cloud/ipfs/QmZAh_SECOND_ACCOUNT",
      gameItem
    )
  );
  nfts.push(
...
describe("NFTVault", async () => {
  let contractVault: Contract,
    contractGameItem: Contract,
    contractOwner: any,
    firstAccount: any,
    secondAccount: any = null;
  let accounts: any[] = [];
  ...
  beforeEach(async () => {
    ...
    [contractOwner, firstAccount, secondAccount] = accounts.map(
      ({ address }) => address
    );
  ...
  describe("Create Vaults", async () => {
    it("Check Permission", async () => {
      // Try deposit without USER role access
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
          .deposit(contractGameItem.address, 123)
      ).to.be.revertedWith("Not authorized");

      // GRANT access as USER role
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
          .grantAccess("USER", firstAccount)
      )
        .to.emit(contractVault, "GrantAccess")
        .withArgs("USER", accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT].address);
    });

    it("Deposit and Withdraw with USER", async () => {
      // GRANT access as USER role
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
      ...
      // Try Approve
      await contractGameItem
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .approve(contractVault.address, 1);
      await contractGameItem
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .approve(contractVault.address, 2);

      // Deposit NFT
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
          .deposit(contractGameItem.address, 1)
      )
        .to.emit(contractVault, "DepositedNft")
        .withArgs(
          accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT].address,
          contractGameItem.address,
          1
        );
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
          .deposit(contractGameItem.address, 1)
      ).to.be.revertedWith("NFT is already in the vaults");

      let nftsFirstAccount = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getVaultsByOwner(firstAccount);
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
          .deposit(contractGameItem.address, 2)
      )
        .to.emit(contractVault, "DepositedNft")
        .withArgs(
          accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT].address,
      ...
      // Value should be 2
      nftsFirstAccount = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getVaultsByOwner(firstAccount);
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT])
          .withdraw(contractGameItem.address, 1)
      ).to.be.revertedWith("Not authorized");
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
          .grantAccess("USER", secondAccount)
      )
        .to.emit(contractVault, "GrantAccess")
      ...
      const index1 = await contractVault.getCompoundIndex(
        contractGameItem.address,
        1
      );
      ...
      const index2 = await contractVault.getCompoundIndex(
        contractGameItem.address,
        2
      );
    ...
    it("Deposit and Withdraw with ADMIN", async () => {
      // GRANT access as USER role
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
          .withdrawByAdmin("0x57f1887a8BF19b14fC0dF6Fd9B2acc9Af147eA85", 0)
      ).to.be.revertedWith("Invalid Address");

      // Value should be 2
      const nftsFirstAccount = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getVaultsByOwner(firstAccount);
      expect(nftsFirstAccount.length).equal(2);
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
          .withdrawByAdmin(contractGameItem.address, 1)
      ).to.be.revertedWith("Not authorized");
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
          .grantAccess("ADMIN", secondAccount)
      )
        .to.emit(contractVault, "GrantAccess")
    ...
    it("Deposit and Withdraw BATCH with USER", async () => {
      // GRANT access as USER role
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
      ...
      const vaultAtempt1 = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getVaultsByOwner(firstAccount);
      ...
      await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .withdrawAll();
      const vault1 = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getVaultsByOwner(firstAccount);
      expect(vault1.length).equal(0);

      // Should be removed all the owners
      const owners = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getOwners();
      expect(owners.length).equal(0);
    });

    it("Deposit and Withdraw BATCH with ADMIN", async () => {
      // GRANT access as ADMIN role for firstAccount
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
          .grantAccess("ADMIN", firstAccount)
      )
        .to.emit(contractVault, "GrantAccess")
      ...
      await contractGameItem
        .connect(accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT])
        .approve(contractVault.address, 3);
      await contractGameItem
        .connect(accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT])
        .approve(contractVault.address, 4);
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT])
          .deposit(contractGameItem.address, 3)
      )
        .to.emit(contractVault, "DepositedNft")
        .withArgs(
          accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT].address,
          contractGameItem.address,
          3
        );
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT])
          .deposit(contractGameItem.address, 4)
      )
        .to.emit(contractVault, "DepositedNft")
        .withArgs(
          accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT].address,
      ...
      const nftsSecondAccount = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.SECOND_ACCOUNT])
        .getVaultsByOwner(secondAccount);
      ...
      let owners = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getOwners();
      ...
      const vault1 = await contractVault
        ...
        .getVaultsByOwner(firstAccount);
      expect(vault1.length).equal(0);
      const vault2 = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getVaultsByOwner(firstAccount);
      expect(vault2.length).equal(0);

      // // Should be removed all the owners
      owners = await contractVault
        .connect(accounts[ACCOUNTS_INDEX.FIRST_ACCOUNT])
        .getOwners();
      expect(owners.length).equal(0);
      ...
      await expect(
        contractVault
          .connect(accounts[ACCOUNTS_INDEX.DEPLOYER])
          .revokeAccess("ADMIN", firstAccount)
      )
